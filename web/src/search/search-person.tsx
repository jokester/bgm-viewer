...
import { Person, PersonSearchResult, PersonsIndexQuery, PersonType } from '../data/api';
  ...
  const formatPersonType = (type: number): string => {
    switch (type) {
      ...
      case PersonType.COMPANY:
        return '公司';
      case PersonType.ASSOCIATION:
        return '组合';
      case PersonType.UNIT:
        return '单位';
      default:
        return '未知';
    }
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <div className='flex items-center gap-4 mt-1 text-xs text-blue-600'>
                  ...
                  {selectedCareer !== null && <span>职业: {selectedCareer}</span>}
                </div>
              </div>
              <Button
                label='修改搜索'
          ...
          <div className='flex justify-between items-center mb-4'>
            <div>
              ...
              <p className='text-sm text-gray-600'>
                显示第 {currentPage * pageSize + 1}-{Math.min((currentPage + 1) * pageSize, totalResults)} 条，共约{' '}
                {totalResults} 条结果
              </p>
            </div>
