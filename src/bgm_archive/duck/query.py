...
class DuckdbQuerier:
    ...
    def find_subject_by_id(self, subject_id: int) -> m.Subject | None:
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Subjects WHERE id = ?", (subject_id,)).df()
            if not len(rows):
                return None
    ...
    def find_character_by_id(self, character_id: int) -> m.Character | None:
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Characters WHERE id = ?", (character_id,)).df()
            if not len(rows):
                return None
    ...
    def find_episodes_by_subject_id(self, subject_id: int) -> list[m.Episode]:
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Episodes WHERE subject_id = ? ORDER BY sort", (subject_id,)).df()
    ...
    def find_subjects_by_ids(self, subject_ids: list[int]) -> list[m.Subject]:
        if not subject_ids:
            return []
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Subjects WHERE id IN ?", (subject_ids,)).df()
    ...
    def find_characters_by_ids(self, character_ids: list[int]) -> list[m.Character]:
        if not character_ids:
            return []
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Characters WHERE id IN ?", (character_ids,)).df()
    ...
    def find_people_by_ids(self, person_ids: list[int]) -> list[m.Person]:
        if not person_ids:
            return []
        with self._db_ref.open_db(read_only=True) as conn:
            rows = conn.execute("SELECT * FROM Persons WHERE id IN ?", (person_ids,)).df()
